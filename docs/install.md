...
4. Installs the gateway as an isolated uv tool, pinned to this release:
   `uv tool install --python 3.12 "abstractgateway[<profile>,tray]==0.4.4"`, from prebuilt wheels
...
uv tool install --python 3.12 "abstractgateway[tray]==0.4.4"    # [apple,tray] or [gpu,tray] for local engines
...
`abstractframework` 0.3.3 pins `abstractgateway==0.4.4`, `abstractassistant==0.5.0`,
`abstractcore==2.15.3`, `AbstractRuntime==0.4.36`, `abstractagent==0.3.13`,
...
| Tool | Command | Version released with 0.3.3 |
|---|---|---|
| Gateway web console | built into `abstractgateway`: open the link `abstractgateway serve` prints (`http://127.0.0.1:8080/console#claim=…`) | 0.4.4 |
| Core web console | built into `abstractcore`: open the link `abstractcore serve` prints (`http://127.0.0.1:8000/console#claim=…`) | 2.15.3 |
